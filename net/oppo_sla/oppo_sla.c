...
/* Fenghua.Xu@PSW.TECH.WiFi.Connect.P00054039, 2019/7/18, add for smart band-width decision, RUS purpose */
/* check WifiRomUpdateHelper.java for value's meaning */
struct oppo_smart_bw_rom_update_info{
...
enum{
	...
	/*Fenghua.Xu@PSW.TECH.WiFi.Connect.P00054039, 2019/7/17 add for smart band-width decision RUS purpose*/
        SMART_BW_SET_PARAMS = 0x39,
...
static struct oppo_smart_bw_rom_update_info smart_bw_rom_update_info;
static bool getSmartBWRomUpdate = false;
static int oppo_smart_bw_set_params(struct nlmsghdr *nlh)
{
	u32* params = (u32 *)NLMSG_DATA(nlh);
	u32 count = NLMSG_PAYLOAD(nlh, NLMSG_HDRLEN); /* this is in fact the payload length which already aligned */
	if (1) {
	        printk("oppo_smart_bw_set_params: (nlh)->nlmsg_len = %u, NLMSG_HDRLEN=%d,"
	                "NLMSG_PAYLOAD(nlh, NLMSG_HDRLEN) = %u, sizeof(oppo_smart_bw_rom) = %lu",
	                (nlh)->nlmsg_len, NLMSG_HDRLEN, count, sizeof(struct oppo_smart_bw_rom_update_info));
	}
#if 0
	if (count == sizeof(struct oppo_smart_bw_rom_update_info)) {
#endif
	        getSmartBWRomUpdate = true;
                ...
                if (smart_bw_rom_update_info.DBG && oppo_sla_debug) {
                        printk("oppo_smart_bw_set_params:set params count=%d params[0] = %d,params[1] = %d,params[2] = %d,params[3] = %d,"
		...
		printk("oppo_smart_bw_set_params:set params invalid param count:%d", count);
	}
#endif
	return	0;
}
...
{
	if (len < sizeof(struct oppo_smart_bw_rom_update_info) / sizeof(int) || !getSmartBWRomUpdate) {
	        if (1) printk("%s target payload len not match or hasn't got ROM update from FWK !\n", __func__);
	        return false;
	}

	if (1) printk("%s enter!\n", __func__);
	memcpy(payload, &smart_bw_rom_update_info, sizeof(struct oppo_smart_bw_rom_update_info));
...
bool get_L3_LQ_status(int payload[], int len)
{
	...
	if (oppo_sla_debug) printk("%s enter!\n", __func__);

	if (oppo_sla_info[MAIN_WLAN].if_up || oppo_sla_info[MAIN_WLAN].need_up ||
	    oppo_sla_info[CELL_INDEX].if_up || oppo_sla_info[CELL_INDEX].need_up) {
	    payload[0] = oppo_sla_info[MAIN_WLAN].avg_rtt;
	    payload[1] = oppo_sla_info[MAIN_WLAN].current_speed;
	    payload[2] = oppo_sla_info[MAIN_WLAN].max_speed;
	    payload[3] = oppo_sla_info[MAIN_WLAN].congestion_flag;

	    payload[4] = oppo_sla_enable;

	    if (oppo_sla_debug) {
	        printk("oppo_sla:get_rtt_speed wlan:max_speed=%d, avg_rtt=%d  "
	               "cell:max_speed=%d, avg_rtt=%d, oppo_sla_enable=%d\n",
	               payload[0], payload[1], payload[2], payload[3], oppo_sla_enable);
	    }
	}
	if (oppo_sla_debug) printk("%s exit!\n", __func__);
	return true;
}
	...
	if (SLA_MODE_DUAL_WIFI == sla_work_mode &&
		...
		if (oppo_sla_info[MAIN_WLAN].download_speed >= MAX_WLAN_SPEED) {
			...
			choose_mark = MAIN_WLAN_MARK;
			 ...
			 ((init_weight_delay_count > 0 && oppo_sla_info[MAIN_WLAN].dl_little_speed < DOWNLOAD_SPEED) ||
			(oppo_sla_info[SECOND_WLAN].download_speed > VIDEO_SPEED && oppo_sla_info[SECOND_WLAN].sla_avg_rtt <= 150)))) {
		...
		if(NULL != th && th->syn &&
			...
			*/
			...
			if(tmp_mark != MAIN_WLAN_MARK) {
				...
				if (dev && !memcmp(dev->name,oppo_sla_info[MAIN_WLAN].dev_name,strlen(dev->name))) {
					if (oppo_sla_debug) {
...
static void statistics_wlan_tcp_tx_rx(const struct nf_hook_state *state,struct sk_buff *skb)
{
	...
	if((oppo_sla_info[MAIN_WLAN].if_up || oppo_sla_info[MAIN_WLAN].need_up) &&
		...
		if(NULL == ct || nf_ct_is_untracked(ct) || (0 == ct->mark)){
			...
			} else if (dev && !memcmp(dev->name,oppo_sla_info[SECOND_WLAN].dev_name,strlen(dev->name))) {
				if (NF_INET_LOCAL_OUT == hook) {
					wlan1_tcp_tx++;
				} else if (NF_INET_LOCAL_IN == hook) {
					wlan1_tcp_rx++;
		...
		if(oppo_sla_info[i].if_up || oppo_sla_info[i].need_up){
			...
			if(oppo_sla_debug /*&& net_ratelimit()*/){
				printk("oppo_sla: dev_name = %s,if_up = %d,max_speed = %d,"
						...
						oppo_sla_info[i].download_speed,oppo_sla_info[i].dual_wifi_download,oppo_sla_info[i].dl_little_speed);
			}
...
calc_weight_finish:
	...
	if(oppo_sla_debug){
		printk("oppo_sla_weight: work_mode = %d,weight[%d] = %d,weight[%d] = %d\n",
				sla_work_mode,index1,oppo_sla_info[index1].weight,index2,oppo_sla_info[index2].weight);
	}
	return;
}
...
finish_calc:
	if(oppo_sla_debug){
		printk("oppo_sla_weight: work_mode = %d,weight[0] = %d,weight[1] = %d,weight[2] = %d\n",
				sla_work_mode,oppo_sla_info[0].weight,oppo_sla_info[1].weight,oppo_sla_info[2].weight);
	}
	return;
}
...
static int wlan_can_enable_second_wifi(int index)
{
	...
	if (oppo_sla_info[index].if_up ||
		...
		if (oppo_sla_info[index].dual_wifi_download >= DOWNLOAD_FLAG &&
			oppo_sla_info[index].download_speed < rom_update_info.dual_wlan_download_speed) {
...
static void sla_show_dailog(void)
{
	...
	if(sla_screen_on &&
		...
		for (i = 0; i < WLAN_NUM; i++) {
			if (oppo_sla_info[i].if_up &&
				oppo_sla_info[i].download_flag < DOWNLOAD_FLAG &&
				oppo_sla_info[i].max_speed <= rom_update_info.sla_speed &&
...
static bool dual_wlan_enable_cell(void)
{
	...
	if (wlan_can_enable_cell(MAIN_WLAN) &&
		...
		sum_speed = oppo_sla_info[MAIN_WLAN].max_speed + oppo_sla_info[SECOND_WLAN].max_speed;
		...
		if(oppo_sla_info[index].if_up || oppo_sla_info[index].need_up){
			...
			if(oppo_sla_debug){
				printk("oppo_sla_rtt: index = %d,wlan_rtt = %d,"
					   ...
					   oppo_sla_enable,oppo_sla_def_net,avg_rtt,sla_detect_mode,sla_work_mode,game_cell_to_wifi);
			}
		}
	}
	sla_rtt_write_unlock();
...
#else
	timer_setup(&sla_timer, (void*)oppo_sla_timer_function, 0);
#endif
